    ...
    session.connect_completed(move |session, success| {
        unsafe {
            if success {
                ...
                let _ = sender_clone.lock().unwrap().try_send(Message::AuthenticationSuccess(window_id));
            } else {
                task.clone().return_result(Err(glib::Error::new(
                    glib::FileError::Failed,
                    "Authentication failed",
                )));
                let _ = sender_clone.lock().unwrap().try_send(Message::AuthenticationFailed(
                    window_id,
...
impl PolkitApp {
        ...
        iced::Subscription::batch(vec![
            iced::Subscription::run(|| {
                iced::stream::channel(100, |sender| {
                    ...
                    async move {
                        let _ = sender_clone.lock().unwrap().try_send(Message::SetSender(sender_clone.clone()));
            ...
            Message::Authenticate(id) => {
                if let Some(session) = self.sessions.get_mut(&id) {
                    ...
                    if let Some(sender) = self.sender.clone() {
                        std::thread::spawn(move || {
                            start_session(&ass, password, task, id, sender);
                        });
                    }
                }
                Command::none()
    ...
    fn view(&self, id: iced::window::Id) -> Element<Message> {
        ...
        if let Some(error) = &session.error {
            content = content
                .push(text(error).size(14).style(|theme| iced::theme::Text::Color(theme.palette().danger)));
        }
